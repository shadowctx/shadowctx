...
export async function generateEmbedding(text: string): Promise<number[] | null> {
  ...
  const trimmed = text.trim().slice(0, 8192)
...
/** Build the text to embed from a page's title and content. */
export function pageEmbeddingText(title: string | null, content: string | null): string {
